...
# 从配置中读取 FastAPI 服务的基础 URL（如 http://localhost:8202）
...
def print_trace_summary(data: dict):
    trace_id = data.get("trace_id")
    if trace_id:
        print(f"trace_id: {trace_id}")


def print_stream_event_meta(event: dict):
    trace_id = event.get("trace_id", "")
    stage = event.get("stage", "")
    elapsed_ms = event.get("elapsed_ms")
    meta_parts = []
    if trace_id:
        meta_parts.append(f"trace_id={trace_id}")
    if stage:
        meta_parts.append(f"stage={stage}")
    if elapsed_ms is not None:
        meta_parts.append(f"elapsed_ms={elapsed_ms}")
    if meta_parts:
                ...
                if t == "token":
                    ...
                    if piece:
                        if debug:
                            token_count += 1
                            sys.stderr.write(
                                f"[token #{token_count} 长度={len(piece)} "
...
if __name__ == "__main__":
    # 创建命令行参数解析器
    parser = argparse.ArgumentParser(description="测试 Agent API（/ask、/intervene、可选 /ask/stream）")
    ...
    parser.add_argument(
        "--question",
        type=str,
        default="请研究 AI Coding Agent 在中小团队研发流程中的典型应用场景，并给出一份面向技术负责人的选型建议。",
        help="测试问题",
    )
    # 解析命令行参数
    ...
    # 打印测试开始信息
    print("开始测试 ResearchFlow Agent API（研究流式版）...\n")
    # 打印后端地址、用户 ID、会话 ID

...
# 导入上下文与响应格式模型
from utils.models import Context, ResearchReport
...
# 导入 tracing 工具，补充 trace_id 与流式事件元信息
from utils.tracing import build_stream_event, create_trace_id, elapsed_ms_since
...
app = FastAPI(
    ...
    description="带有检索、流式输出与人工审批能力的研究型 Agent API 服务",
    version="1.0.0",
    lifespan=lifespan
)
...
def _coerce_research_report(payload: Any) -> Optional[ResearchReport]:
    if payload is None:
        return None
    if isinstance(payload, ResearchReport):
        return payload
...
def _format_research_report(report: ResearchReport) -> str:
    summary = report.summary.strip()
    steps = "\n".join(f"{idx}. {step}" for idx, step in enumerate(report.steps, 1))
    sources = "\n".join(
        f"- {source.title} | {source.url} | {source.note}" for source in report.sources
    ) if report.sources else "- 暂无明确来源"
    next_actions = "\n".join(
        f"{idx}. {action}" for idx, action in enumerate(report.next_actions, 1)
    )
    return (
        f"研究摘要:\n{summary}\n\n"
        f"执行步骤:\n{steps}\n\n"
...
def _extract_completion_payload(result_like: Dict[str, Any]) -> Dict[str, Any]:
    report = _coerce_research_report(result_like.get("structured_response"))
    if report is not None:
        return {
            "result": _format_research_report(report),
            "sources": [source.model_dump() for source in report.sources],
        }

    messages = result_like.get("messages") or []
    if messages:
...
def _build_interrupt_details(hitl_req: Any) -> Dict[str, Any]:
    return {
...
def _infer_interrupt_stage(interrupt_details: Dict[str, Any]) -> str:
    action_requests = interrupt_details.get("action_requests", [])
    if any(action.get("name") == "search_documents" for action in action_requests):
        return "retrieval"
    return "planning"


def _build_streamed_interrupt_event(hitl_req: Any, trace_id: str, started_at: float) -> Dict[str, Any]:
    interrupt_details = _build_interrupt_details(hitl_req)
    return build_stream_event(
        "interrupted",
        {
            "interrupt_details": interrupt_details,
            "stage": _infer_interrupt_stage(interrupt_details),
        },
        trace_id=trace_id,
...
def _build_streamed_completion_event(
...
async def run_agent_with_hitl(
    ...
    if "__interrupt__" in result:
        ...
        completion_payload = _extract_completion_payload(result)
        return {"status": "completed", **completion_payload, "trace_id": trace_id}
        ...
        if stream_mode == "messages":
            ...
            if node == "tools":
                yield build_stream_event(
                    "tool_output",
                    {"content": content, "stage": "tool_output"},
                    trace_id=trace_id,
                    elapsed_ms=elapsed_ms_since(started_at),
                )
            else:
                yield build_stream_event(
                    "token",
                    {"content": content, "stage": "planning"},
                    trace_id=trace_id,
                    elapsed_ms=elapsed_ms_since(started_at),
                )
        # 对于状态更新流（updates 类型）
            ...
            if "__interrupt__" in chunk:
                ...
                hitl_req = hitl_requests[0]
                yield _build_streamed_interrupt_event(hitl_req, trace_id=trace_id, started_at=started_at)
                return
    # 全部流程结束后，主动获取当前 agent state
    state = await agent.aget_state(config)
    if state and state.values:
        yield _build_streamed_completion_event(
            _extract_completion_payload(state.values),
            trace_id=trace_id,
...
async def ask(request: AskRequest):
    ...
    logger.info(f"/ask接口接收用户问题并启动 Agent 执行，trace_id：{trace_id} 用户ID： {request.user_id} 会话ID： {request.thread_id} 用户问题： {request.question}")

    # 为本次请求创建独立 Agent 实例
    agent = await create_agent_instance()

    # 读取该用户的长期记忆（如用户名等）
    user_memory = await read_long_term_info(request.user_id)
    ...
    messages = chat_prompt.format_messages(
        ...
        user_memory=user_memory or "暂无长期记忆",
        expected_audience="AI 应用开发实习岗位候选人",
    )
    # 取最后一条作为用户消息
...
async def ask_stream(request: AskRequest):
    ...
    # 记录流式请求日志
    logger.info(f"/ask/stream trace_id: {trace_id} 用户ID: {request.user_id} 会话ID: {request.thread_id} 问题: {request.question}")
    # 创建 Agent 实例
    agent = await create_agent_instance()
    # 读取长期记忆
    user_memory = await read_long_term_info(request.user_id)
    ...
    async def event_generator():
        async for event in run_agent_with_hitl_streaming(
            agent,
            human_msg.content,
            config,
            context,
...
async def intervene(request: InterveneRequest):
    trace_id = create_trace_id()
    # 记录介入请求日志
    logger.info(f"/intervene接口接收人工提交决策并继续执行被中断的 Agent，trace_id：{trace_id} 用户ID： {request.user_id} 会话ID： {request.thread_id} 人工决策反馈数据： {request.decisions}")
    ...
    # 无新中断：取最后一条消息内容为最终回答
    completion_payload = _extract_completion_payload(result)
    logger.info(f"Agent最终回复是: {completion_payload['result']}")
    return AgentResponse(status="completed", trace_id=trace_id, **completion_payload)
...
async def intervene_stream(request: InterveneRequest):
    ...
    started_at = time.perf_counter()
    # 记录流式恢复请求日志
    logger.info(f"/intervene/stream trace_id: {trace_id} 用户ID: {request.user_id} 会话ID: {request.thread_id} 决策数: {len(request.decisions)}")
    # 创建 Agent 实例
    agent = await create_agent_instance()
    # 构造 config
    config = {
    ...
    async def event_generator():
        async for event in run_agent_resume_streaming(
            agent,
            config,
            context,
            request.decisions,

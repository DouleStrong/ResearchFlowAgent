...
def _format_search_result_records(items) -> str:
    filtered_result_string = ""
    for title, content_chunk, link, pubAuthor, pubDate, distance in items:
        record = (
            f"文章标题: {title}\n"
            f"文章原始链接: {link}\n"
            f"文章发布者: {pubAuthor}\n"
    ...
    try:
        ...
        if filter_result["success"]:
            ...
            if filter_result["results"] and len(filter_result["results"]) > 0:
                ...
                # 将过滤搜索结果拼接成字符串
                filtered_result_string = _format_search_result_records(filtered_items)
                # 打印完整的搜索结果字符串
